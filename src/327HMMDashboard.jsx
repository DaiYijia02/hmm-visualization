...
const EnhancedHMMVisualizer = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    matrix_a: 'all', // This will now represent {entropy + second largest eigenvalue}
    matrix_b: 'all',
    pi: 'all'
  });
  ...
  // Function to normalize matrix string for consistent comparison
  const normalizeMatrixString = (matrixStr) => {
    if (!matrixStr) return '';
    // Remove whitespace, quotes, and convert to lowercase for consistent matching
    return matrixStr.replace(/[\s'"]+/g, '').toLowerCase();
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // First load the matrix properties from 327matrix.csv
        const matrixPropertiesMap = {};
        
        try {
          const matrixResponse = await fetch('./data/327-matrix.csv');
          if (!matrixResponse.ok) {
            console.warn("Matrix properties file not available:", matrixResponse.status);
          } else {
            const matrixContent = await matrixResponse.text();
            
            // Parse matrix properties CSV
            Papa.parse(matrixContent, {
              header: true,
              dynamicTyping: true,
              skipEmptyLines: true,
              complete: (results) => {
                // Create a mapping of matrix strings to their properties
                results.data.forEach(row => {
                  if (!row.As) return; // Skip if no matrix
                  
                  try {
                    // Parse eigenvalues
                    let eigenValues = [];
                    if (row.eigen_values) {
                      eigenValues = parseList(row.eigen_values);
                    }
                    
                    // Get second largest eigenvalue
                    const secondLargestEigen = eigenValues.length > 1 ? eigenValues[1] : 0;
                    
                    // Get entropy
                    const entropy = typeof row.entropies === 'number' ? row.entropies : 0;

                    // Get pi_0s
                    const pi_0s = row.pi_0s;
                    
                    // Create a normalized key for the matrix
                    const normalizedKey = normalizeMatrixString(row.As);
                    
                    // Store the properties
                    matrixPropertiesMap[normalizedKey] = {
                      entropy,
                      secondLargestEigen,
                      // Add pi_0s from the matrix file
                      pi_0s: pi_0s,
                      // Combined category for filtering
                      category: `E:${entropy.toFixed(1)} λ:${secondLargestEigen.toFixed(2)}`
                    };
                  } catch (e) {
                    console.warn("Error processing matrix row:", e);
                  }
                });
                
                console.log(`Loaded ${Object.keys(matrixPropertiesMap).length} matrix properties`);
              }
            });
          }
        } catch (error) {
          console.error("Error loading matrix properties:", error);
          // Continue with main data loading even if matrix properties loading fails
        }
        
        // Load the main CSV file
        const response = await fetch('./data/327-Qwen2.5-1.5B.csv');
        if (!response.ok) {
        ...
        Papa.parse(fileContent, {
          ...
          complete: (results) => {
            ...
            const processedData = results.data.map(row => {
              // Get A matrix and normalize for lookup
              const aMatrix = row.A;
              const normalizedAMatrix = normalizeMatrixString(aMatrix);
              
              // Look up matrix properties or use defaults from the main file
              const aProps = matrixPropertiesMap[normalizedAMatrix];
              
              // Get matrix_a category - either from the matrix properties or calculate from data
              let matrix_a_cat;
              if (aProps) {
                // If we found matching properties, use the combined category
                matrix_a_cat = aProps.category;
              } else {
                // Otherwise, try to extract from the main file's fields
                let entropy = row.A_entropy || 0;
                let secondLargestEigen = 0;
                
                // Try to get second largest eigenvalue
                if (row.eigen_value) {
                  try {
                    const eigenValues = parseList(row.eigen_value);
                    secondLargestEigen = eigenValues.length > 1 ? eigenValues[1] : 0;
                  } catch (e) {
                    console.warn("Error parsing eigenvalues from main file:", e);
                  }
                }
                
                matrix_a_cat = `E:${entropy.toFixed(1)} λ:${secondLargestEigen.toFixed(2)}`;
              }
              
              // Process B matrix as before
              ...
              const matrixProps = matrixPropertiesMap[normalizedAMatrix] || {};
              
              return {
                ...row,
                // Use our new combined category
                matrix_a_cat,
                matrix_b_entropy: bEntropy,
                matrix_b_cat: getEntropyCategory(bEntropy),
                pi_cat: row.pi?.includes("1.0") ? "Deterministic" : "Uniform",
                matrix_a_pi_0s: matrixProps.pi_0s
              };
            });
            ...
            const cats = {
              ...
              matrix_a: [...new Set(processedData.map(row => row.matrix_a_cat).filter(Boolean))].sort(),
              matrix_b: [...new Set(processedData.map(row => row.matrix_b_cat).filter(Boolean))].sort(),
              pi: [...new Set(processedData.map(row => row.pi_cat).filter(Boolean))]
            };
            setCategories(cats);
  ...
  // Get pi_0s for a selected A matrix
  const getSelectedMatrixPi0s = () => {
    if (filters.matrix_a === 'all') {
      return null; // No specific matrix selected
    }
    
    // Find an example row that matches the selected matrix_a
    const matchingRow = data.find(row => row.matrix_a_cat === filters.matrix_a);
    return matchingRow.matrix_a_pi_0;
  };
    ...
    <div className="hmm-visualizer">
      ...
      <div className="data-summary">
        ...
        <div className="data-summary-grid">
          ...
          {/* Display pi_0s when a specific A matrix is selected */}
          {getSelectedMatrixPi0s() && (
            <div className="data-summary-item matrix-info">
              <h3>Selected A Matrix Initial State (π₀)</h3>
              <p className="data-summary-pi">{getSelectedMatrixPi0s()}</p>
            </div>
          )}
        </div>
      </div>
...
export default EnhancedHMMVisualizer;