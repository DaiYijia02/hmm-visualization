...
const ExtendHMMDashboard = () => {
  // Dataset selection
  const [selectedDataset, setSelectedDataset] = useState('expand_obs');
  
  // State for data and filters
  const [expandObsData, setExpandObsData] = useState([]);
  const [expandStateData, setExpandStateData] = useState([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [expandObsResponse, expandStateResponse] = await Promise.all([
          fetch('./data/Qwen2.5-7B_expand_obs.csv'),
          fetch('./data/Qwen2.5-7B_expand_state.csv')
        ]);
        
        const expandObsContent = await expandObsResponse.text();
        const expandStateContent = await expandStateResponse.text();
        
        // Parse the expand_obs CSV
        Papa.parse(expandObsContent, {
          header: true,
          dynamicTyping: true,
          complete: (results) => {
            const processedData = processCSVData(results);
            setExpandObsData(processedData);
            
            // Parse the expand_state CSV
            Papa.parse(expandStateContent, {
              header: true,
              dynamicTyping: true,
              complete: (results) => {
                const processedData = processCSVData(results);
                setExpandStateData(processedData);
                
                // Set filter options based on the selected dataset
                setFilterOptions(
                  extractFilterOptions(selectedDataset === 'expand_obs' ? processedData : expandObsData)
                );
                
                setLoading(false);
              },
              error: (error) => {
                setError(`Error parsing expand_state CSV: ${error.message}`);
                setLoading(false);
              }
            });
          },
          error: (error) => {
            setError(`Error parsing expand_obs CSV: ${error.message}`);
            setLoading(false);
          }
        });
      } catch (error) {
  ...
  useEffect(() => {
    if (expandObsData.length > 0 && expandStateData.length > 0) {
      setFilterOptions(
        extractFilterOptions(selectedDataset === 'expand_obs' ? expandObsData : expandStateData)
      );
      
      // Log some sample data to debug column names
      const sampleData = selectedDataset === 'expand_obs' ? expandObsData[0] : expandStateData[0];
  ...
  }, [selectedDataset, expandObsData, expandStateData]);
  
  // Get the current dataset
  const currentData = selectedDataset === 'expand_obs' ? expandObsData : expandStateData;
  ...
  const groupEntropyCategories = (data) => {
    ...
    const createRanges = (values) => {
      ...
      // round values to the nearest 0.5, and remove duplicates
  ...
  return (
    <div className="container">
      <h1 className="title">Extend HMM Results Visualization</h1>
        ...
        <div className="dataset-options">
          <label className="radio-item">
            <input
              ...
              name="dataset"
              value="expand_obs"
              checked={selectedDataset === 'expand_obs'}
              onChange={() => setSelectedDataset('expand_obs')}
            />
            <span>Expand Observations</span>
          </label>
          <label className="radio-item">
            <input
              type="radio"
              ...
              name="dataset"
              value="expand_state"
              checked={selectedDataset === 'expand_state'}
              onChange={() => setSelectedDataset('expand_state')}
            />
            <span>Expand States</span>
          </label>
        </div>
      </div>
      ...
      <div className="info-panel">
        <p>
          <strong>Current Selection:</strong> {selectedDataset === 'expand_obs' ? 'Expand Observations' : 'Expand States'} dataset,
...
export default ExtendHMMDashboard;