...
const GeneralSteadyState = () => {
  ...
  const [availableSteadyStates, setAvailableSteadyStates] = useState([]);
  ...
  useEffect(() => {
    ...
    if (configMap[key]) {
      ...
      setAvailableSteadyStates(availableValues);
      
      // If current steady state is not in the list or not set, select the first one
      if (!availableValues.includes(steadyState) || steadyState === null) {
        setSteadyState(availableValues[0]);
      }
    } else {
      setAvailableSteadyStates([]);
      setSteadyState(null);
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`./data/Qwen2.5-7B_11111_4096_steady_state_2048.csv`);
        ...
        parsedData.data.forEach(row => {
          ...
          // Extract first element of steady state array with 0.01 precision
          const steadyStateValue = getFirstSteadyStateElement(row.steady_state);
          
          if (steadyStateValue !== null && !configMapping[key].includes(steadyStateValue)) {
            configMapping[key].push(steadyStateValue);
          }
        });
        
        // Sort steady state values within each configuration
  ...
  useEffect(() => {
    if (allData.length === 0 || steadyState === null) return;
    
    const matchingData = allData.find(item => 
      item.config.num_state === numState && 
      item.config.num_observation === numObservation && 
      Math.abs(item.config.steady_state_first - steadyState) < 0.0001 && // Use approximate equality for floating point
    ...
    <div className="visualization-container">
      ...
      <div className="notes-section">
        ...
        <ul className="notes-list">
          ...
          <li>Steady State first element represents the probability of being in the first state at equilibrium</li>
...
export default GeneralSteadyState;